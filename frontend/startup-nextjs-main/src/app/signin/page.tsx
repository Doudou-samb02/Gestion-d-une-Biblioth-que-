...
import { useAuth } from "../context/AuthContext"; // chemin à adapter selon ton projet
...
export default function SigninForm() {
  ...
  const { login } = useAuth(); // 🔹 récupère login depuis AuthContext
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const user = await login(email, password); // login renvoie l'user
      if (user.role === "ADMIN") {
